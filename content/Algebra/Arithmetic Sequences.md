...
>The value of the $k$th term $a_k$ and the common difference $d$ of an arithmetic sequence are related to the value of the $n$th term $a_n$ via the formula below. 
>$$a_n = a_k + d(n - k)$$
...
>[!check]- Solution 2
>In this sequence, the first term is $a_1 = -2$ and the common difference is $5,$ so the recursive formula is as follows.
>$$
>\boxed{
>\begin{cases}
>a_1 = -2 \\
...
>53 = -5 + 2(N-1) \\
>58 = 2 (N-1) & \text{add 5} \\
>29 = N - 1 & \text{divide by 2} \\
>N = 30 & \text{add 1}
>\end{gather*}
>$$ 
>Now, use the formula for the sum of the terms of an arithmetic sequence with $N = 30$ in order to determine the sum.
>$$
>(-5) + (-3) + (-1) + \cdots + 53 = \frac{30}{2} (-5 + 53) = \boxed{720}
>$$