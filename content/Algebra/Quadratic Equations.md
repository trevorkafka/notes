...
| 3. [Quadratic Formula](#quadratic-formula)         | can be used to solve any quadratic equation                                                                                                                              | requires careful arithmetic and simplification; the slowest method when factoring is possible |
## Factoring
...
$$
In certain situations, both of these values may be the same (ultimately amounting to just one solution) or potentially neither may be real values. More details about this can be found in the [Nature of Solutions](#nature-of-solutions) section.
...
>The **discriminant** of a quadratic expression $ax^2 + bx + c$ is defined to be the value $b^2 - 4ac.$

The discriminant is used to determine the number and nature of the number of solutions to the quadratic equation, as described in the next section.
...
>
>**Case 1:** $b^2 - 4ac > 0.$ In this case, $\sqrt{b^2 - 4ac}$ will be some positive (real) number. Thus, the quadratic formula reads 
>$$
...
>$$
>Hence, this represents two distinct imaginary values for $x$ that form a complex conjugate pair.

### Sum of Solutions

>[!note] The Sum and Average of the Solutions to a Quadratic Equation
...
> $$
> The value of $x_\text{avg}$ matches (1) the equation of the axis of symmetry $x = - \frac{b}{2a}$ and (2) $x$-coordinate of vertex of the corresponding parabola with equation $y = ax^2 + bx + c.$ In the case that there is only one solution to the quadratic equation, the value of $x_\text{avg}$ is equal to that single solution.
...
> $$
> While $k$ is currently unknown, imagine it to be chosen such that this equation can be satisfied for only one possible value of $x$. Determine the possible values of $k$ and their corresponding $x$ solutions.
...
> x_\times = \frac{c}{a}
> $$
> When the quadratic equation has only one solution, the above value is the square of the single solution $x = - \frac{b}{2a}$. This product value $x_\times$ in this case can be thought of as the result of multiplying the solution $x = - \frac{b}{2a}$ by itself (it is a *double root*, after all).
