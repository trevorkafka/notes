...
Now, notice that the numerator is the derivative of the denominator, so we can use the pattern $\displaystyle \int \frac{u'}{u}\ dx = \ln|u| + C.$

$$\boxed{\ln|\sec x + \tan x| + C}$$
# Method 2: substitution $u = \sec x$

Let $u = \sec x$, so $du = \sec x \tan x\ dx,$ which gives $\dfrac{1}{\tan x}\ du = \sec x\ dx$. Since $\tan x = \sqrt{\sec^2 x - 1}$, we can hence write $\dfrac{1}{\sqrt{u^2 - 1}} \ du = \sec x\ dx$, which can be put into the integral.
$$
