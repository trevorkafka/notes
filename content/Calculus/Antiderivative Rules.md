...
# Basic Functions
Let $a$ and $n$ be arbitrary constants.
$$
\begin{align*}
&\int a\ dx &&= ax + C \\
&\int x^n\ dx &&= \frac{1}{n+1}\ x^{n+1} + C & \text{(power rule, for $n \neq -1$)}
\end{align*}
$$
The power rule cannot be used when $n = -1$, since it would require division by zero. That case, $\int \frac{1}{x}\ dx$, is covered in the next section.
