...
>[!note] Theorem: Differentiability Implies Continuity
>If a function $f(x)$ is differentiable at $x = a$ then it also must be continuous at $x = a.$

The converse is *not* true: a function can be continuous at a point without being differentiable there. The classic example is $f(x) = |x|$ at $x = 0$, which is continuous but has a sharp corner there, so the sided derivatives $f_-'(0) = -1$ and $f_+'(0) = 1$ disagree and $f'(0)$ does not exist.