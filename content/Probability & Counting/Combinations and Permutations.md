...
The number of ways to **permute** $r$ out of $n$ total items is written as ${}_nP_r$ and can be computed using either an explicit formula involving [[Factorials|factorials]] or built-in calculator functionality.
...
The number of ways to **combine** $r$ out of $n$ total items is written as ${}_nC_r$ or as $\binom n r$ and can be computed using either an explicit formula using [[Factorials|factorials]] or built-in calculator functionality. In contexts where the notation $\binom n r$ is used, the term **binomial coefficient** is also used, which stems from its relationship with the binomial theorem.

>[!important] Explicit formula for combinations (a.k.a. - binomial coefficient)
>
>$$
>\binom n r = {}_nC_{r} = \frac{n!}{(n-r)!\ r!}
>$$

Note that by this formula we can establish the identity ${}_nP_r = r! \cdot {}_nC_r,$ which also implies that ${}_nP_r \geq {}_nC_r,$ meaning that *the number of combinations cannot exceed the number of permutations.*
