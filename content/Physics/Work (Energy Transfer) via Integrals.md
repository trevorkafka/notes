...
> $$
> \begin{gather*}
> 7.5\ \text{ft} \cdot \text{lb} = \int_{0\ \text{ft}}^{\frac{1}{6}\ \text{ft}} kx\ dx \\
> 7.5\ \text{ft} \cdot \text{lb} = \left. \frac{1}{2}kx^2 \right|_{0\ \text{ft}}^{\frac{1}{6}\ \text{ft}} \\
> 7.5\ \cancel{\text{ft}} \cdot \text{lb} = \frac{k}{72} \text{ft}^{\cancel 2} \\
> k = 540\ \frac{\text{lb}}{\text{ft}}
> \end{gather*}
> $$
> Now, use this value of $k$ to compute the *additional* work done in compressing the spring from $x_1 = 2\ \text{in} = \frac{1}{6}\ \text{ft}$ to $x_2 = 2.5\ \text{in} = \frac{5}{24}\ \text{ft}$. Note that the lower limit of this integral is $\frac{1}{6}\ \text{ft}$ rather than $0\ \text{ft}$, since the first $2$ inches of compression have already been accounted for by the $7.5\ \text{ft} \cdot \text{lb}$ given in the problem.
> $$
> \begin{gather*}
> W = \int_{\frac{1}{6}\ \text{ft}}^{\frac{5}{24}\ \text{ft}} \left( 540\ \frac{\text{lb}}{\text{ft}} \right) x\ dx \\
> W = \left. 270\ \frac{\text{lb}}{\text{ft}} \cdot x^2 \right|_{\frac{1}{6}\ \text{ft}}^{\frac{5}{24}\ \text{ft}} \\
> W = 270\ \frac{\text{lb}}{\text{ft}} \left( \frac{25}{576} - \frac{16}{576} \right) \text{ft}^2 \\
> W = \frac{135}{32}\ \text{ft-lb} \approx \boxed{4.22\ \text{ft-lb}}
> \end{gather*}
> $$
...
>
><span class="invert">![[cone-tank-pumping-figure.png]]</span>
>
...
> 
> <span class="invert">![[cone-tank-cross-section.png]]</span>
> 
...
> W = \int_{0\ \text{ft}}^{6\ \text{ft}} 62.4 \cdot \frac{4\pi y^2}{9} \cdot (6-y)\ dy \\
> W = 62.4 \cdot \frac{4\pi}{9} \int_0^6 6y^2 - y^3\ dy \\
> W = \left. 62.4 \cdot \frac{4\pi}{9} \left( 2y^3 - \frac{1}{4}y^4 \right) \right|_0^6 \\
